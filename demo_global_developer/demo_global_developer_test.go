...
func isNonDecrease(nums []int) bool {
	if len(nums) == 1 {
		return true
	}
	var count int
	for i := 1; i < len(nums); i++ {
		if nums[i] < nums[i-1] {
			if i == 1 {
				...
				if count == 1 {
					return false
				} else if (nums[i] >= nums[i-2]) || (nums[i] < nums[i-2] && nums[i+1] >= nums[i-1]) {
					count += 1
				} else {
					return false
				}
...
func TestIsNonDecrease(t *testing.T) {
	a := []int{5, 7, 1, 8}
	expected := true
	res := isNonDecrease(a)
	if res != expected {
